...
COLOR_MAP = {
    ...
    "Activision Blizzard": "#1565C0",              # bleu pétrole (différent d'Ubisoft)
...
# Sélecteurs communs (répliqués par onglet)
...
with tabs[0]:
    ...
    if dfp.empty:
        ...
        # Calcul dynamique de la taille (racine carrée pour compresser l’échelle)
        dfp["taille_bulle"] = np.sqrt(dfp["payroll"].clip(lower=0)) * (12 * size_scale)
        color_map = _get_color_map_for(dfp, "Editeur")

        # Customdata pour le hover : [Editeur, CA, Profit, Payroll]
        dfp = dfp.assign(
            _ed=dfp["Editeur"],
            _ca=dfp["ca"],
            _profit=dfp["profit"],
        ...
        fig = px.scatter(
            ...
            custom_data=["_ed", "_ca", "_profit", "_pay"],
        ...
        fig.update_layout(height=420, margin=dict(l=50, r=30, t=60, b=50), legend_title_text="",
        ...
        # Tooltip correct : affiche l'éditeur + valeurs numériques formatées
        fig.update_traces(
            hovertemplate="<b>%{customdata[0]}</b><br>"
                          "CA: %{customdata[1]:,.0f} M€<br>"
                          "Résultat: %{customdata[2]:,.0f} M€<br>"
                          "Masse salariale: %{customdata[3]:,.0f} M€<extra></extra>".replace(",", " ")
        )
        st.plotly_chart(fig, use_container_width=True)
...
with tabs[1]:
    ...
    if dfp2.empty:
        ...
        fig2.update_layout(height=420, margin=dict(l=50, r=30, t=60, b=50), legend_title_text="",
                           plot_bgcolor="white", paper_bgcolor="white")
        ...
        fig2.update_traces(
            hovertemplate="<b>%{customdata[0]}</b><br>"
                          "Effectif: %{customdata[1]:,.0f}<br>"
                          "Masse salariale: %{customdata[2]:,.0f} M€<extra></extra>".replace(",", " ")
        )
